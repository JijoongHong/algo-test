...
public class P2098 {
    ...
    public static void main(String[] args) throws Exception {
        ...
        System.out.println(dfs(0, 1)); // dp[0][1] 0번 정점에서 자기자신을 방문한 상태일 때, 다른 모든 정점을 방문해서 다시 0번 정점으로 돌아가는 최소비용
    }
    ...
    public static int dfs(int cur, int visit) {
        if (visit == allVisited) // 모든 정점 방문
            return adj[cur][0] == 0 ? INF : adj[cur][0]; // 현재위치에서 0번 정점으로 돌아갈 수 있는가? 이미 0번이 Visit처리된 것 다룸
        if (dp[cur][visit] != -1) // 현재 정점에서 나머지를 방문한적있는가?
            return dp[cur][visit];
        ...
        for(int i = 0; i < N; i++) {
            int next = 1 << i;
            if(adj[cur][i] != 0 && ((visit & next) == 0)) { // 가지않은 곳
                dp[cur][visit] = Math.min(dp[cur][visit], dfs(i, (visit|next)) + adj[cur][i]);
                // min 이유 : 어떤 다른 정점을 거쳐서 가는게 유리할 수 있으므로
            }
        }
        return dp[cur][visit];
    }
